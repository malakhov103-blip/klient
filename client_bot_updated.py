...
BOT_TIMEZONE_NAME = clean_env("BOT_TIMEZONE", "Europe/Prague")

try:
    BOT_TIMEZONE = ZoneInfo(BOT_TIMEZONE_NAME)
except ZoneInfoNotFoundError as error:
    raise RuntimeError(
        f"Неизвестный часовой пояс BOT_TIMEZONE: {BOT_TIMEZONE_NAME}. "
        "Используйте, например, Europe/Prague."
    ) from error
...
class AtomicPicklePersistence(PicklePersistence):
    """Сохраняет прежний формат памяти, но пишет атомарно и держит backup."""
...
class PerUserUpdateProcessor(BaseUpdateProcessor):
    """Обслуживает разных клиентов параллельно, одного клиента — по очереди."""
    ...
    async def do_process_update(self, update, coroutine):
        ...
        key = ("user", user.id) if user is not None else (("chat", chat.id) if chat is not None else ("service", 0))
...
STEPS = [
    ...
    {
        "key": "household", "kind": "single", "emoji": "🤝",
        "text": "Кем приходятся друг другу будущие жильцы?",
        "options": ["Буду проживать один/одна", "Пара", "Семья",
                    "Друзья / знакомые", "Коллеги"],
        "label": "Состав проживающих", "allow_other": True,
    },
    {
...
def get_move_in_options(reference_date=None):
    """Возвращает ближайшие варианты заезда по календарю в часовом поясе бота."""
    current_date = reference_date or datetime.now(BOT_TIMEZONE)
    current_month_index = current_date.month - 1
    next_month_name = RU_MONTHS_GENITIVE[(current_month_index + 1) % 12]
    following_month_name = RU_MONTHS_GENITIVE[(current_month_index + 2) % 12]
    return [
        "Как можно скорее",
        f"С {next_month_name}",
        f"С {following_month_name}",
    ]
...
CLIENT_KEYS = ["age", "people", "household", "from_country", "how_long", "citizenship",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if effective_chat is not None and getattr(effective_chat, "type", "private") != "private":
        await update.message.reply_text(
            "Пожалуйста, заполните заявку в личном чате с ботом. "
            "Откройте @binio_client_bot и отправьте /start."
        )
        return ConversationHandler.END
    context.user_data.clear()
    await update.message.reply_text(
...
async def privacy_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Коротко объясняет клиенту назначение собранных ответов."""
    await update.message.reply_text(
        "ℹ️ Ответы в анкете нужны для подбора жилья и связи с вами по вашему запросу.\n\n"
        "После отправки заявка поступает менеджеру Binio в чат заявок. "
        "Если вы хотите уточнить, исправить или удалить свои данные, напишите администратору Binio "
        "через контакт, указанный в официальном канале."
    )
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        fallbacks=[
            ...
            CommandHandler("privacy", privacy_info),
            MessageHandler(filters.COMMAND, unknown_command),
        ],
        persistent=True,
        # В v2 добавлен новый шаг. Новое имя не даёт незавершённым анкетам
        # старой версии попасть в неправильный вопрос после смены индексов.
        name="client_survey_v2",
        allow_reentry=True,
    )

    application.add_handler(conv_handler)
    application.add_handler(CommandHandler("privacy", privacy_info))
    ...
    # Не удаляем заявки/ответы, которые пришли во время короткого перезапуска.
